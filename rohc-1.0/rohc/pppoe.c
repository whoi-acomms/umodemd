 ...
 * Version:    0.6.11
 *
 * 230503 :     Modification by the ROHC Project 2003 to get correct
 *              header size.
...
int pppoe_rcv_core(struct sock *sk, struct sk_buff *skb)
{
	...
	if (sk->state & PPPOX_BOUND) {
		struct pppoe_hdr *px = (struct pppoe_hdr*)skb->data;
		// Changes by ROHC to use header length
		int l = ntohs(px->length);
