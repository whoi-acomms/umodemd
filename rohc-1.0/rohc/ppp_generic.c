 ...
 *
 * Modified by the ROHC Project 2003 at Lulea University of Technology,
 * Sweden.
 *
 * ==FILEVERSION 20030523==
 */
...
struct ppp {
	...
	void *		rohc_comp;	/* rohc compressor state */
	void *		rohc_decomp;	/* rohc decompressor state */
		     ...
		     unsigned int cmd, unsigned long arg)
{
	...
	case PPPIOCSMAXCID:
		...
		printk(KERN_INFO "vj init, val2=%d val=%d\n", val2+1, val+1);
	...
	case PPPIOROHCCID:
		...
		printk(KERN_INFO "rohc init cid=%d\n", val);
		printk(KERN_INFO "rohc old: %p %p",
		    ppp->rohc_comp, ppp->rohc_decomp);
		comp = rohc_alloc_compressor(val);
		if (!comp) {
			printk(KERN_ERR "ROHC Compressor initialization failed\n");
			err = -ENOMEM;
			break;
		} else
			printk(KERN_INFO "ROHC Compressor initialized\n");

		decomp = rohc_alloc_decompressor(comp);
		if (!decomp) {
			printk(KERN_ERR "ROHC Decompressor initialization failed\n");
			rohc_free_compressor(comp);
			err = -ENOMEM;
			break;
		} else
			printk(KERN_INFO "ROHC Decompressor initialized\n");

		rohc_proc_set(comp, decomp);

		ppp_lock(ppp);
		if (ppp->rohc_comp)
			rohc_free_compressor(ppp->rohc_comp);
		if (ppp->rohc_decomp)
			rohc_free_decompressor(ppp->rohc_decomp);
		ppp->rohc_comp = comp;
		ppp->rohc_decomp = decomp;
	...
	case PPPIOROHCMRRU:
		if (get_user(val, (int *) arg))
			break;
		printk(KERN_INFO "rohc mrru=%d\n", val);

		ppp_lock(ppp);
		rohc_c_set_mrru(ppp->rohc_comp, val);
		ppp_unlock(ppp);
		err = 0;
		break;

	case PPPIOROHCHEADER:
		if (get_user(val, (int *) arg))
			break;
		printk(KERN_INFO "rohc header=%d\n", val);

		ppp_lock(ppp);
		rohc_c_set_header(ppp->rohc_comp, val);
		ppp_unlock(ppp);
		err = 0;
		break;

	case PPPIOROHCPROFILE:
		if (get_user(val, (int *) arg))
			break;
		printk(KERN_INFO "rohc profile=%d\n", val);

		ppp_lock(ppp);
		rohc_activate_profile(ppp->rohc_comp, val);
		ppp_unlock(ppp);
		err = 0;
		break;
	default:
...
static inline char toAscii(unsigned char c) {
	if (c >= '0' && c<='z')
		return c;
	return ' ';
}

#define NICE_DUMP_LEVEL 0

static void niceDump(char *str, struct sk_buff *skb, int level) {
	int i,j;

	if (level > NICE_DUMP_LEVEL)
		return;
	...
	for (i=0; i<skb->len; i+=8) {
		...
		for (j=0; j<8; j++) {
			if (i+j<skb->len)
				printk("%02x ", skb->data[i+j]);
			else
				printk("   ");
		}
		for (j=0; j<8; j++) {
			if (i+j<skb->len)
				printk("%c", toAscii(skb->data[i+j]));
			else
				printk(" ");
		}
		printk("\n");
...
ppp_send_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	case PPP_IP:
		...
		if (ppp->rohc_comp && ppp->rohc_decomp && rohc_c_is_enabled(ppp->rohc_comp)) {
		//if (0) {
			//printk(KERN_INFO "mtu=%d hard=%d\n", ppp->dev->mtu, ppp->dev->hard_header_len);
			//printk(KERN_ERR "rohc_comp 1\n");

			niceDump("Going to compress PPP_IP packet", skb, 2);

			new_skb = alloc_skb(ppp->dev->mtu, GFP_ATOMIC);

			if (!new_skb) {
				printk(KERN_ERR "ROHC: No memory for compression\n");
				ppp->rohc_comp = 0;
				ppp->rohc_decomp = 0;
				break;
			}
			...
			rohc_length = rohc_compress(ppp->rohc_comp,
				skb->data + 2, skb->len - 2,                  // in
				new_skb->data + 2, skb_tailroom(new_skb)-2);  // out

			if (rohc_c_using_small_cid(ppp->rohc_comp)) {
				new_skb->data[0] = PPP_SMALL_ROHC >> 8;
				new_skb->data[1] = PPP_SMALL_ROHC & 0xff;
			} else {
				new_skb->data[0] = PPP_LARGE_ROHC >> 8;
				new_skb->data[1] = PPP_LARGE_ROHC & 0xff;
			}

			skb_put(new_skb, 2+rohc_length); 

			niceDump("Sending PPP_ROHC packet", new_skb, 2);
...
#endif /* CONFIG_PPP_MULTILINK */
		//printk(KERN_INFO "non multilink..\n");
...
ppp_receive_nonmp_frame(struct ppp *ppp, struct sk_buff *skb)
{
	...
	case PPP_LARGE_ROHC:
		rohc_large_cid = 1;
	case PPP_SMALL_ROHC:
		niceDump("PPP_ROHC Received", skb, 1);
		//printk(KERN_ERR "PPP_ROHC Received\n");

		ns = dev_alloc_skb(ppp->dev->mtu);
		if (ns == 0) {
			printk(KERN_ERR "ROHC D: No memory\n");
			goto err;
		}

		skb_reserve(ns, 2);

		len = rohc_decompress_both(ppp->rohc_decomp, skb->data + 2,
			skb->len - 2, ns->data + 2,
			skb_tailroom(ns), rohc_large_cid);

		if (len < 20) { // error
			printk(KERN_INFO "PPP_ROHC Decompress error (%d)\n", len);
		  //printk(KERN_INFO "ROHC Decomp: Error value=%d\n", len);
			//niceDump("ROHC packet", skb, 0);

			//ns->len = 20;
			//niceDump("Failed IP packet", ns, 0);
		...
		ns->data[0] = PPP_IP >> 8;
		ns->data[1] = PPP_IP & 0x00ff;
		proto = PPP_IP;

		skb_put(ns, 2+len);
		//niceDump("Decompressed IP packet", ns, 2);

		kfree_skb(skb);
		skb = ns;

		//printk(KERN_ERR "PPP_ROHC Decompress done..\n");

		break;

	case PPP_IP:
		//printk(KERN_INFO "len=%d\n", skb->len-2);
		//niceDump("PPP_IP Received", skb, 0);
		break;
...
static void ppp_destroy_interface(struct ppp *ppp)
{
	...
	if (ppp->rohc_comp) {
		printk(KERN_INFO "ROHC: freeing compressor\n");
		rohc_free_compressor(ppp->rohc_comp);
		ppp->rohc_comp = 0;
	}
	if (ppp->rohc_decomp) {
		printk(KERN_INFO "ROHC: freeing decompressor\n");
		rohc_free_decompressor(ppp->rohc_decomp);
		ppp->rohc_decomp = 0;
	}
	rohc_proc_set(NULL,NULL);
