...
while /bin/true; do
	...
	# extract the packet data (IP header, transport-layer header, payload)
	# from TMPFILE. Get rid of whitespace and newlines. Put into raw_modem
	# file.
        grep 0x[0-9]*: TMPFILE | cut -d: -f2-9 | sed 's/[ \t]//g' | tr -d '\n' | tr -d '\r' >> ${LOGFILE}.raw_modem

	# convert ${LOGFILE}.modem to ${LOGFILE}.nmea_modem:
        # a.) Use Routing Table to convert DEST_IPV4_ADDRESS to next-hop modem
	#     SRC address.
	# b.) Add CCL IP_Payload type.
	# c.) If Path MTU Discovery worked, then TMPFILE data should fit in
	#     one frame.
	#   Generate legacy $CCCYC/$CCTXD commands.
	#   Write ${LOGFILE}.nmea_modem
	# A modified umodemd would then monitor a given directory for
	# new *.nmea_modem files, and move them into the transmitted directory/

	mv ${LOGFILE} ${LOGFILE}.raw_modem transmitted/
